...
"""Tests for Howdoi."""
import os #importing functions from os.
import unittest #i guess it is a set of built in functions for testing 

from howdoi import howdoi  #opening howdoi.py from howdoi folder


class HowdoiTestCase(unittest.TestCase): #according to me it is creating a class for testing all the scripts in it.

    def call_howdoi(self, query):
        parser = howdoi.get_parser() #this is related to argparse.
        args = vars(parser.parse_args(query.split(' '))) #this is again related argarse but here .split(" ") converts a string into a list. 
        return howdoi.howdoi(args) #returns not prints

    def setUp(self):
        self.queries = ['format date bash', #it is related to pyquery
